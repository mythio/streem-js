...
const encObject = (obj: any): Buffer => {
	...
	for (const key in obj) {
		data = Buffer.concat([data, encodeUString(key), encodeOne(obj[key])]);
	}
...
export const encodeAmf0Cmd = (opt: any): Buffer => {
	...
	if (rtmpCmd[opt.cmd]) {
		rtmpCmd[opt.cmd].forEach((prop: string) => {
			if (opt.hasOwnProperty(prop)) data = Buffer.concat([data, encodeOne(opt[prop])]);
		});
	}
